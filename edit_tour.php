<?php
	...
	$tourid = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INT);

	if (!$tourid) {
		die("Érvénytelen ID.");
	}
	
	$query = "SELECT category, length, quantity, date, price, place, region, description FROM tours WHERE id = ?";
	...
	if (isset($_POST['upload-btn'])) {
		...
		if (empty($category) || empty($length) || empty($quantity) || empty($date) || empty($price) || empty($place) || empty($region) || empty($description)) {
			die("Minden mező kitöltése kötelező!");
		}
		
		$stmt = $conn->prepare("UPDATE tours SET category = ?, length = ?, quantity = ?, date = ?, price = ?, place = ?, region = ?, description = ? WHERE id = ?");
		if (!$stmt) {
			die("Hiba az előkészített lekérdezésben: " . $conn->error);
		}
		$stmt->bind_param("ssssssssi", $category, $length, $quantity, $date, $price, $place, $region, $description, $tourid);
    ...
    <div id="main-wrap" class="full-width">
		...
		<div id="page-content" class="header-static footer-fixed">
            <div id="flexslider" class="fullpage-wrap small">
                <ul class="slides">
                    <li style="background-image:url(assets/img/tour.jpg)">
                        <div class="container text text-center">
                            <h1 class="white margin-bottom-small">Túráink</h1>
                            <p class="heading white">Túráinkat kiváló szakemberekkel egyeztetve szervezzük meg. Minden szombaton lehetőségetek adódik részt venni engedélyköteles vagy szabadon bejárható lelőhely közös felfedézésére.</p>
                        </div>
                        <div class="gradient dark"></div>
                    </li>
                    <ol class="breadcrumb">
                        <li><a href="index.php">Főoldal</a></li>
                        <li><a href="tours.php" class="active">Túrák</a></li>
                    </ol>
                </ul>
            </div>
			<div id="home-wrap" class="content-section fullpage-wrap row grey-background">
                <div class="container">
					...
					<form id="contact-form" class="padding-onlytop-md padding-md-topbottom-null" action="add_tour.php?id=<?php echo $tourid; ?>" method="post">
					<div class="row">
						<div class="col-md-12">
							<input class="form-field" name="category" id="category" type="text" placeholder="Kategória" value="<?php echo htmlspecialchars($category); ?>" required>
						</div>
						<div class="col-md-12">
							<input class="form-field" name="length" id="length" type="text" placeholder="Hossz/óra" value="<?php echo htmlspecialchars($length); ?>" required>
						</div>
						<div class="col-md-12">
							<input class="form-field" name="quantity" id="quantity" type="text" placeholder="Résztvevők/fő" value="<?php echo htmlspecialchars($quantity); ?>" required>
						</div>
						<div class="col-md-12">
							<input class="form-field" name="date" id="date" type="date" value="<?php echo htmlspecialchars($date); ?>" required>
						</div>
						<div class="col-md-12">
							<input class="form-field" name="price" id="price" type="text" placeholder="Ár/fő" value="<?php echo htmlspecialchars($price); ?>" required>
						</div>
						<div class="col-md-12">
							<input class="form-field" name="place" id="place" type="text" placeholder="Helyszín" value="<?php echo htmlspecialchars($place); ?>" required>
						</div>
						<div class="col-md-12">
							<input class="form-field" name="region" id="region" type="text" placeholder="Régió" value="<?php echo htmlspecialchars($region); ?>" required>
						</div>
						<div class="col-md-12">
							...
							<div class="col-md-12">
								<div class="submit-area padding-onlytop-sm">
									<input type="submit" class="btn-alt" name="upload-btn" value="Túra Módosítása">
								</div>
							</div>
						</div>
					</form>
						<?php
							...
							function getGalleryJSON($conn) {
								$tourid = $_GET['id'];
								$query = "SELECT `image` FROM `tours` WHERE `id` = $tourid ";
								$result = $conn->query($query);
								$row = $result->fetch_assoc();
								return json_decode($row['image'], true);
							}

							function updateGalleryJSON($conn, $galleryArray) {
								$tourid = $_GET['id'];
								$galleryJSON = $conn->real_escape_string(json_encode($galleryArray));
								$query = "UPDATE `tours` SET `image` = '$galleryJSON' WHERE `id` = $tourid ";
								$conn->query($query);
							}
